...
class TestZeekAnalysisHandler(unittest.TestCase):
    ...
    def test_start_static_analysis(self, mock_run, mock_thread_class, mock_glob):
        ...
        expected_calls = [
            call(
                target=mock_run,
                args=(["zeek", "-r", "/mock/static/test1.pcap", "/mock/config.zeek"],),
            ),
            call(
                target=mock_run,
                args=(["zeek", "-r", "/mock/static/test2.pcap", "/mock/config.zeek"],),
            ),
        ]
    ...
    def test_start_network_analysis(self, mock_run, mock_thread_class, mock_popen):
        ...
        mock_thread_class.assert_any_call(
            target=mock_run, args=(["zeekctl", "deploy"],)
        )
        ...
        read_output_call = any(
            "read_output" in str(arg[1]) for arg in mock_thread_class.call_args_list
        )
