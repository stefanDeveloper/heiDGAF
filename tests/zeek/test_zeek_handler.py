...
class TestZeekHandler(unittest.TestCase):
    ...
    def test_setup_zeek_success(
        ...
        result = runner.invoke(
            setup_zeek,
            ["-c", "/mock/config.yaml", "--zeek-config-location", "/mock/zeek.cfg"],
        )

        # Assert
        mock_config_handler_cls.assert_called_once_with("config_data", "/mock/zeek.cfg")
    ...
    def test_setup_zeek_with_error(
        self,
        mock_yaml_safe_load,
        mock_copy,
        mock_open,
    ...
    @patch("shutil.copy2")
    def test_setup_zeek_static_analysis(
        self, mock_copy, mock_open, mock_analysis_handler, mock_config_handler_cls
    ):
        # Arrange
    ...
    @patch("shutil.copy2")
    def test_default_config_location_used(
        self, mock_copy, mock_open, mock_analysis_handler, mock_config_handler_cls
    ):
        """
        ...
        mock_config_handler_cls.assert_called_once_with(
            "config_data", "/usr/local/zeek/share/zeek/site/local.zeek"
        )
    ...
    @patch("shutil.copy2")
    def test_custom_config_location_used(
        self, mock_copy, mock_open, mock_analysis_handler, mock_config_handler_cls
    ):
        """
        ...
        result = runner.invoke(
            setup_zeek,
            ["-c", "/mock/config.yaml", "--zeek-config-location", "/custom/zeek.cfg"],
        )

        # Assert
        # Verify that custom location was used
        mock_config_handler_cls.assert_called_once_with(
            "config_data", "/custom/zeek.cfg"
        )
    ...
    @patch("shutil.copy2")
    def test_non_initial_setup_restore_backup(
        self,
        mock_copy,
        mock_isfile,
        mock_open,
        mock_analysis_handler,
        mock_config_handler_cls,
    ):
        """
        ...
        mock_copy.assert_any_call(
            "/opt/local.zeek_backup", "/usr/local/zeek/share/zeek/site/local.zeek"
        )
    ...
    @patch("os.path.isfile")
    @patch("shutil.copy2")
    def test_initial_setup_backup_default(
        self,
        mock_copy,
        mock_isfile,
        mock_open,
        ...
        mock_copy.assert_any_call(
            "/usr/local/zeek/share/zeek/site/local.zeek", "/opt/local.zeek_backup"
        )
