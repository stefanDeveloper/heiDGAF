...
class TestRelevanceHandler(unittest.TestCase):
    def setUp(self):
        ...
        self.list_item_instance = ListItem(
            name="status_code",
            allowed_list=["200", "300", "404"],
            relevant_list=["200", "404"],
        )
        ...
        self.handler = RelevanceHandler(
            log_configuration_instances=self.log_config_instances
        )

    def test_initialization(self):
        self.assertEqual(
            self.handler.log_configuration_instances, self.log_config_instances
        )
    ...
    def test_check_relevance_with_invalid_function(self):
        ...
        self.assertIn(
            "Function non_existing_function is not implemented", str(context.exception)
        )
