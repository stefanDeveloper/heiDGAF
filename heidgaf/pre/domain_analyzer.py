...
class DomainAnalyzer(Analyzer):
    ...
    def run(self, data: pl.DataFrame, redis_cache: DataFrameRedisCache) -> pl.DataFrame:
        ...
        _, warning = self.update_count(self, df, "fqdn", self.KEY_FQDN, redis_cache)
        ...
        _, warning = self.update_count(
            ...
            df,
            "secondleveldomain",
            self.KEY_SECOND_LEVEL_DOMAIN,
            redis_cache,
        )
        self.set_warning(self, df, warning, "secondleveldomain")

        _, warning = self.update_count(
            self, df, "thirdleveldomain", self.KEY_THIRD_LEVEL_DOMAIN, redis_cache
        )
